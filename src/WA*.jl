...
function pathFindingWAstar(map::Matrix{Path},D::Tuple{Int64,Int64},A::Tuple{Int64,Int64},w::Float64)
    ...
    P::PriorityQueue = PriorityQueue{Tuple{Int64,Int64},Float64}(Base.Order.Forward)  
        ...
        for i in eachindex(next)
            ...
            if !(visited[u[1],u[2]][1]) && !(visited[u[1],u[2]][2]) && traversable(map[u[1],u[2]].type) #si non visiter et non ferme alors
                #si la case est traversable alors on l'ajoute a la Priority Queue
                    ...
                    enqueue!(P,u,w*convert(Float64,dist[u[1],u[2]]) + (1-w)*convert(Float64,heuristique(u,A)))
                    push!(Q,u)
    ...
    return (vectorWay(D,point,prev),Q,numberBoxVisited,dist[point[1],point[2]])
end
